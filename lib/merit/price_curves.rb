module Merit
  ...
  module PriceCurves
    ...
    class LastLoaded < FirstUnloaded
      ...
      # given +point+.
      def producer_at(point)
        dispatch = @order.participants.dispatchables
        loaded   = dispatch.reject { |producer| producer.load_at(point).zero? }

        if loaded.any?
          loaded.max_by { |producer| producer.cost_at(point) }
        else
          dispatch.first
        end
      end
      ...
      def fallback_producer(point)
        ...
        dispatch.reverse_each.detect do |producer|
